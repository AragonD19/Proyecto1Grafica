                ...
                if (y>0 && y<SCREEN_HEIGHT && x>0 && x<SCREEN_WIDTH) {
                    ...
                    if (isBarycentricCoord(barycentricCoord)) {
                        ...
                        if (depth < zBuffer[index]) {
                            ...
                            Color fragmentShaderf = fragmentShaderMercury(fragment); //Cambiar nombre del método para ver los shaders
...
int main(int argc, char* args[]) {
    ...
    window = SDL_CreateWindow("Lab4", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_SHOWN);
    ...
    bool success = loadOBJ("../modelos/Esfera.obj", vertices, normal, faces);
    if (!success) {
        return 1;
    }
